...
from mysql_insert import insert_user_to_mysql,insert_bookingdata_to_mysql,insert_ratingdata_to_mysql
...
def handle_message(event):
    ...
    if mtext == '@圖片地圖':
        ...
        manage_rating_Form(event, mtext,username)
    elif mtext[:3] == '///' and len(mtext) > 3:
        manage_booking_Form(event, mtext,username)
...
def manage_rating_Form(event, mtext,username):
    try:
        flist = mtext[3:].split('/')
        mname=flist[0]
        rate=flist[1]
        comment=flist[2]
        text1 = '用戶名：{}\n電影名稱：{}\n評分：{}\n評語：{}\n已將您評分資料傳入資料庫！'.format(username,mname,rate,comment)
        insert_ratingdata_to_mysql(username,mname,rate,comment)
        message = TextSendMessage(
            text = text1
        )
        line_bot_api.reply_message(event.reply_token,message)
...
def manage_booking_Form(event, mtext,username):
    try:
        flist = mtext[3:].split('/')
        mname=flist[0]
        session=flist[1]
        text1 = '用戶名：{}\n電影名稱：{}\n場次：{}\n已登記您的訂票資訊！'.format(username,mname,session)
        insert_bookingdata_to_mysql(username,mname,session)
